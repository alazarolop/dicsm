NbClustPar <- function (data = NULL, diss = NULL, distance = "euclidean", 
  min.nc = 2, max.nc = 15, method = NULL, index = "all", alphaBeale = 0.1) 
{
  ...
  if (any(indice == 27) || (indice == 31) || (indice == 32)) {
    nc.Hubert <- 0
    indice.Hubert <- 0
    pic[["Hubert_stat"]] <- ggplot(mapping = aes(x = x_axis, y = res[, 27])) +
      geom_point(colour = "blue") +
      geom_line(colour = "blue") +
      labs( x = "Number of clusters",
            y = "Hubert Statistic values")
    pic[["Hubert_diff"]] <- ggplot(mapping = aes(x = DiffLev[, 1], y = DiffLev[, 10])) +
      geom_point(colour = "red") +
      geom_line(colour = "red") +
      labs( x = "Number of clusters",
            y = "Hubert statistic second differences")
  ...
  if (any(indice == 29) || (indice == 31) || (indice == 32)) {
    nc.Dindex <- 0
    indice.Dindex <- 0
    pic[["Dindex_stat"]] <- ggplot(mapping = aes(x = x_axis, y = res[, 29])) +
      geom_point(colour = "blue") +
      geom_line(colour = "blue") +
      labs( x = "Number of clusters",
            y = "Dindex values")
    pic[["Dindex_diff"]] <- ggplot(mapping = aes(x = DiffLev[, 1], y = DiffLev[, 12])) +
      geom_point(colour = "red") +
      geom_line(colour = "red") +
      labs( x = "Number of clusters",
            y = "Second differences Dindex values")
  ...
  if ((indice == 14) || (indice == 15) || (indice == 16) || 
    ...
    results.final <- list(All.index = res, All.CriticalValues = resCritical, 
      Best.nc = resultats, Best.partition = partition, "pic" = pic)
  }
